...
describe('movementXp — weight, intensity, regeneration', () => {
  it('60 min strength (weight 1.1, medium) → 15 XP (spec example)', () => {
    expect(
      movementXp({ durationMin: 60, weight: 1.1, intensity: 'medium', isRegeneration: false }),
    ).toBe(15);
  });

  it('120 min hiking (weight 1.05) → round(15×1.05)=16 (spec example)', () => {
    expect(
      movementXp({ durationMin: 120, weight: 1.05, intensity: 'medium', isRegeneration: false }),
    ).toBe(16);
  });

  it('45 min yoga (weight 1.0, medium) → 12 (spec example)', () => {
    expect(
      movementXp({ durationMin: 45, weight: 1.0, intensity: 'medium', isRegeneration: false }),
    ).toBe(12);
  });

  it('15 min walk (weight 1.0) → 6 (spec example)', () => {
    expect(
      movementXp({ durationMin: 15, weight: 1.0, intensity: 'medium', isRegeneration: false }),
    ).toBe(6);
  });

  it('regeneration is a fixed 6 XP regardless of duration', () => {
    expect(movementXp({ durationMin: 5, weight: 1.0, intensity: null, isRegeneration: true })).toBe(
      6,
    );
    expect(
      movementXp({ durationMin: 90, weight: 1.1, intensity: 'intense', isRegeneration: true }),
    ).toBe(6);
  });

  it('intensity is a small factor and never outweighs the duration bucket', () => {
    const light = movementXp({
      durationMin: 60,
      weight: 1.1,
      intensity: 'light',
      isRegeneration: false,
    });
    const intense = movementXp({
      durationMin: 60,
      weight: 1.1,
      intensity: 'intense',
      isRegeneration: false,
    ...
    const shorter = movementXp({
      durationMin: 60,
      weight: 1.0,
      intensity: 'medium',
      isRegeneration: false,
    });
    const longer = movementXp({
      durationMin: 90,
      weight: 1.0,
      intensity: 'medium',
      isRegeneration: false,
...
describe('ritualCheckinRawXp', () => {
  it('daily blocks are 2 XP each', () => {
    expect(ritualCheckinRawXp(['daily_block', 'daily_block', 'daily_block'])).toEqual({
      dailyXp: 6,
      specialXp: 0,
    });
  });
  it('special actions are 5 XP and tracked separately', () => {
    ...
    expect(ritualCheckinRawXp(['daily_block', 'special_action'])).toEqual({
      dailyXp: 2,
      specialXp: 5,
    });
  });
