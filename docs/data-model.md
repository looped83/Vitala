...
base_rule (Verweis, konstant im Code/Config), is_active. _(Referenzdaten, kein
Household)_
...
- Index(household_id, occurred_on); Index(activity_group_id).
  **`activity_participants`** — id, activity_group_id, user_id, household_id.
  _(verknüpft gemeinsame Aktivität; Stadt-XP nur einmal je Gruppe – erzwungen in RPC.)_
  **`ritual_definitions`** — id, key, area (`nutrition`|`sustainability`|`animal_welfare`),
  ...
  _(Referenzdaten: Ernährungsbausteine, Nachhaltigkeits-/Tierwohl-Handlungen.)_
...
auf Ursprung), occurred_on, created_at. _(Append-only; Level = Aggregat.)_

- Index(household_id, scope), Index(user_id, scope).
  **`level_definitions`** — scope (`personal`|`city`), level, cumulative_xp, title(null).
  _(Referenzdaten; generiert aus Formel resources-and-xp §3/§4.)_
  ...
  **`resource_transactions`** — id, household_id FK, resource_key, amount (int, +/−),
  ...
  source_id, created_by(user_id null), idempotency_key, created_at. _(Append-only Ledger;
  `resources.balance` = Summe, per Trigger konsistent gehalten oder als Materialized/RPC.)_
...
is_swappable, is_active. _(Referenz/Template-Pool.)_
...
- Index(household_id, period_start).
  ...
  **`goal_progress`** — id, goal_id FK, period_start, period_end, current_value,
  is_completed, computed_at. _(Snapshot je Periode; Live-Wert wird zusätzlich aus Einträgen
  aggregiert.)_
...
a11y_description, is_v1. _(Referenzdaten.)_
...
required (int), updated_at. _(Fortschritt je Ressource; Refund = Summe invested.)_
**`world_areas`** — id, household_id FK, area_key, unlocked (bool), unlocked_at(null),
min_city_level. _(Stadtbereiche.)_
...
created_at. _(Statische Deko/Natur; datengetriebene Weltrekonstruktion.)_
...
Kontext, **keine** Roh-Gesundheitsdaten). _(Stadtgeschichte.)_

### 16.6 Auszeichnungen & Audit

**`achievements`** — id, key, name, description, condition (jsonb), scope
(`personal`|`household`). _(Referenzdaten; optionale Meilenstein-Abzeichen, freundlich.)_
...
before (jsonb null), after (jsonb null), created_at. _(kritische Mutationen: Rollen,
Household-Änderungen, Löschungen, manuelle Korrekturen.)_
